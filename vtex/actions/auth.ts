...
import { getVtexConfig, vtexFetchWithCookies, vtexFetchResponse } from "../client";
...
export async function classicSignIn(
  ...
  const body = new URLSearchParams({ email, password, authenticationToken: token });
...
export async function sendEmailVerification(
  ...
  locale?: string,
  parentAppId?: string,
): Promise<{ success: boolean; authenticationToken: string | null; setCookies: string[] }> {
  try {
    let token = authenticationToken;
